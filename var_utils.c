...
/**
 * validate_variable_chain - checks we should continue chaining based on last status
 * @info: the parameter struct
 * @buf: the char buffer
 * @p: address of current position in buf
 ...
 * Return: Void
 */
void validate_variable_chain(info_t *info, char *buf, size_t *p, size_t i, size_t len)
{
	size_t j = *p;
